...
// 250000
// 0
// console.log("Bài 1 của Hiếu");

// function cleanPrice(rawString) {
//   if (!rawString) {
//     return 0;
//   } else {
//     let stringClean = rawString.replace(/\D/g, "");
//     let rawNumber = Number(stringClean);
//     return rawNumber;
//   }
// }
...
function cleanPrice(rawString) {
  ...
  let stringClean = rawString
  .replace("Giá:", "")
  .replace("VND", "")
  .replaceAll(".", "")
  .trim();
  return Number(stringClean);
}
let sp1 = "  Giá: 1.500.000 VND  ";
let sp2 = "   250.000 VND";
let sp3 = "";
...
function taoBaoCaoTest(testRuns) {
  ...
  for (let item of testRuns) {
    const [rawTestName, { browser, env }, rawStatus] = item;
    let testName = rawTestName.trim();
    let browserClean = browser.trim();
    let envClean = env.trim();
    ...
    let result = `${testName} - ${browserClean} - ${envClean}`;
    if (status === "PASS") {
      pass.push(result);
    } else {
      fail.push(result);
...
//   slowestRespone: 3400
// }
console.log("Bài config của Hiếu");
...
let tagConfig = [...tagsMacDinh, ...tagsThem];
let tagClean = [];
for (let item of tagConfig) {
  let itemClean = item.trim().toLowerCase();
  if (itemClean === "") {
    continue;
  } 
  if (!tagClean.includes(itemClean)) {
    tagClean.push(itemClean);
  }
}
...
function taoCauHinhChayTest() {
  ...
  config: configCuoi,
  tag: tagClean,
  slowestRespone: Math.max(...tocDoPhanHoi)
  }
}
