...
// neu trong then cos loi thì nó sẽ dừng lại ở lỗi đầu, còn nếu có catch ở cuối thì dừng ở cuối
...
// Hoặc dùng return Promise - vì đã return ra kết quả thì catch sẽ bắt đc
new Promise((resolve) => {
  resolve("OK");
})
  .then(() => {
...
async function muaHangOnline() {
    console.log("Bắt đầu đăng nhập trang web");
    let trangWeb = await moTrangWeb("neko.com");
  console.log(trangWeb);

    let canDangNhap = await dangNhap(trangWeb, "neko", "neko@123");
  console.log(canDangNhap);
    let themVaoGio = await themVaoGioHang(canDangNhap, "Khoá học 1");
  console.log(themVaoGio);
    let thanhToanGio = await thanhToan(themVaoGio);
  console.log(thanhToanGio);
}
muaHangOnline();
