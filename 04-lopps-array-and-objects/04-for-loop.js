...
console.log("Bài 2 của Hiếu");
for (kiemTraUrl of danhSachUrl) {
  console.log(`Đang kiểm tra: ${kiemTraUrl}`);
...
// gia:30000
// ---
console.log("Bài 3 của Hiếu");
for (let objectProduct of products) {
  for (let key in objectProduct) {
    console.log(`${key}: ${objectProduct[key]}`);
  }
  console.log("-----");
...
// Trần Kim Hoàng (18478617) 3:01 PM
...
//in ra snr phẩm còn hàng
console.log("bài 4 của Hiếu");
for (let sanPham of sanPhams) {
  if (!sanPham.conHang) {
    continue;
    // console.log(`Đã tìm thấy sản phẩm hết hàng: ${sanPham.ten}`);
  } 
  console.log(`Sản phẩm còn hàng ${sanPham.ten}`);
}


// Trần Kim Hoàng (18478617) 3:21 PM
...
console.log("Bài 5 của Hiếu");
for (let userArray in userInfo) {
  if (fieldAnDi.includes(userArray)) {
    continue;
  } else {
    console.log(`${userArray}: ${userInfo[userArray]}`);
  }
}
...
let danhSachGia = [5000, 120000, 800000, 30000, 25000, 50000];

//Yêu cầu
// tạo 1 array rỗng danhSachVip
/// Nếu giá > 100000 thì ta cho sản phẩm vào dnah sách vip
// và làm tiếp yêu cầu đề bài, là in ra danh sách vip và số
console.log("Bài 6 của Hiếu");
let danhSachVip = [];
...
// Trần Kim Hoàng (18478617) 3:46 PM
// let danhSachVip = [];
